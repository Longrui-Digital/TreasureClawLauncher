...
def fetch_manifest(url: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        append_exception_log_line("[更新] 無法取得更新清單", e)
        return None
...
def _download_to_path_stream(
    ...
    try:
        ...
        append_exception_log_line("[更新] 下載失敗", e)
        return False
...
def _launcher_update_with_progress_ui(
    ...
    def worker() -> None:
        try:
            ...
            append_exception_log_line("[更新] 套用失敗", e)
            result_ok[0] = False
            q.put(("done", False))
    ...
    try:
        ...
        append_exception_log_line(
            f"[更新] 無法建立 Tk 視窗（{type(e).__name__}），改為無視窗下載", e
        )
        if not _launcher_apply_manifest_with_progress(man, lambda _p: None):
            print("[啟動] 更新失敗，仍嘗試啟動目前版本")
            return launch_main_script(root)
